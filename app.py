...
def validate_run_spec(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "optimize_file_loading" in payload:
        if not isinstance(payload["optimize_file_loading"], bool):
...
app.include_router(
    build_report_router(
        ...
        list_candidate_base_urls=lambda backtest_id: get_active_base_urls(),
    )
)
