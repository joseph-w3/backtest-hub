...
- 模块划分:
  ...
  - `scripts/submit_run.py`: 生成/读取 run_spec 并调用 `/runs` 提交；可选 WebSocket 跟随日志并落盘。
  - `scripts/backtest_hub_sdk.py`: 查询 backtest 状态/下载日志的 CLI 工具。
...
- 关键职责:
  ...
  - `scripts/submit_run.py`: 调用 `generate_run_spec.py`（可选）并以 multipart 方式上传策略与 run_spec；`--follow-logs` 时连接 `/runs/backtest/{backtest_id}/logs/stream` 并写入 `./live_logs/{backtest_id}.log`。
  - `scripts/backtest_hub_sdk.py`: `get-backtest-status` 调用 `/runs/backtest/{backtest_id}`，`download-backtest-logs` 调用 `/runs/{backtest_id}/logs`。
- 主要依赖:
...
- 数据处理链路:
  ...
  4) 读取 runner 脚本，与 run_spec/策略一并转发至 backtest docker。
...
- 日志流链路:
  ...
  - `scripts/submit_run.py --follow-logs` 通过 WebSocket 拉取日志并落盘到 `./live_logs/{backtest_id}.log`。
...
- 运行步骤:
  ...
  2) 调用 `POST /runs` 上传 run_spec 与策略文件（需 `X-API-KEY`）。
  ...
  7) 可选：连接 `/runs/backtest/{backtest_id}/logs/stream` 实时查看日志（或 `scripts/submit_run.py --follow-logs` 自动落盘）。
