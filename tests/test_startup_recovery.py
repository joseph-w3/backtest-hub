...
class TestRecoverSubmittedTasks(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_succeeded_task_recovered_as_completed(self) -> None:
        """A submitted task whose worker reports 'succeeded' gets status=completed."""
        with tempfile.TemporaryDirectory() as td:
            store = self._setup_store(td)
            store.upsert_run("bt_succ", {
                "status": "submitted",
                "backtest_api_base": "http://worker1:10001",
                "required_memory_gb": 4.0,
            })
